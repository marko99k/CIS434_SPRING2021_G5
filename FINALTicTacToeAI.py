# Tic Tac Toe game aganist the computer
import random
choice = ''
playerLetterChoice = ''
computerLetterChoice = ''

# Declaration in order to keep track of modes and player wins/ lose status
mode = ''
player1wins = 0
player2wins = 0
bestOfGames = 0
...
# Tic-Tac-Toe algorithm for the computer to determine win
    ...
    for i in range(1, 10):
        ...
        if freeSpace(replica, i):
            ...
            if gameWinner(replica, playerplayerLetter):
                return i
# Player Modes added: Easy, Medium,and Hard
    if(mode == 'easy'):
...
while True:
# Count of how many games are chosen and the level of difficulty
    mode = input('Enter easy, medium or hard')
    bestOfGames = int(input('How many rounds do you wish to play? (best of 1, 3,5,7,etc.)'))
    while(bestOfGames > player1wins and bestOfGames > player2wins ):
        tBoard = [' '] * 10
        playerschoice()
        turn = firstPlayer()
    ...
    #Computer wins the game:
                 if gameWinner(tBoard, computerLetterChoice):
                     creategameBoard(tBoard)
                     print('The computer is the winner! You have lost the game!')
...
#If no rematch is chosen:
    if (player1wins > player2wins):
        print('The player has won the game '+ str(player1wins) + '-' + str(player2wins))
    if (player2wins > player1wins):
        print('The computer has won the game ' + str(player2wins) + '-' + str(player1wins))
    if not rematchGame():  
        break
    activeGame = True
    player1wins = 0
